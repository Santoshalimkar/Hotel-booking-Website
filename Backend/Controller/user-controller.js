...
await property.save();
      ...
      res.redirect(`https://lonavala-stay.netlify.app/paymentsuccessful?&orderid=${razorpay_order_id}&proid=${propertyId}&checkin=${checkIn}&checkout=${checkOut}&amount=${amountpaid}`);
    } catch (error) {
      console.error(error);
      res.status(500).json({ message: "Internal server error" });
    }
